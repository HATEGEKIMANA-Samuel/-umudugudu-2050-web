...
if (isset($_GET['cre'])){
	$course = encrypt_decrypt('decrypt', $_GET['cre']);
}
...
if (!isset($_POST['id_to_edit'])){
	if ($name =='' || $year =='') {
	    ...
	    $sql= "INSERT INTO courses(name,year,description,diplomat,time,user,place) 
	                       VALUES('$name','$year','$description',$diplomat,NOW(),{$_SESSION["id"]},'{$place}')";
	    if ($database->query($sql)) {
	        $id=rawurlencode(encrypt_decrypt('encrypt', $diplomat)).rawurlencode("%courses");
    ...
    $sql = "UPDATE courses SET name='$name',year='$year',description='$description',place='{$place}' WHERE id=$id";
    if ($database->query($sql)) {
        $dpl=rawurlencode(encrypt_decrypt('encrypt', $diplomat)).rawurlencode("%courses");
        ...
        <div class="tab-container">
            <div class="tab-content mt-10" id="nav-tabContent">
                    ...
                    <?php 
						...
						$text = "Adding";
						if (isset($course) && is_numeric($course)) {
	                    		$query =$database->query("SELECT * FROM courses WHERE id = '$course' AND status='1' LIMIT 1");
                ...
                <div class="tab-content-body">
                   <h4 class="mt-20 fs-18" style="font-family: georgia;text-align: center;margin-bottom: 20px;"><?=$text?> <span style="color:green;"><?php echo "{$diplomat_data['given_name']} {$diplomat_data['family_name']}"; ?></span>'s Course <hr class="mt-20 mb-10"></h4>
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" id="form">
                	<?php echo $messages;?>
                	<?php if (isset($course) && is_numeric($course)) {?>
                		<input type="hidden" name="id_to_edit" value="<?php echo $course; ?>" />
                	<?php }?>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" maxlength="255" placeholder="Course name" name="name"  value="<?php if (isset($course) && is_numeric($course)){echo $row['name'];}elseif(isset($_POST['name'])){echo $_POST['name'];}?>">
                  </div>
                  

                  <div class="form-group">
                    <label for="name">Where Taken<span class="required-mark">*</span></label>
                    <input type="text" class="form-control" maxlength="255" placeholder="Where The Course Is Taken" name="place" value="<?php if (isset($course) && is_numeric($course)){echo $row['place'];}elseif(isset($_POST['place'])){echo $_POST['place'];}?>">
                  </div>
                  
                  <div class="form-group">
                    <label for="country">Year<span class="required-mark">*</span></label>
                        <select class="form-control" name="year">
                        <option value="">--Choose course's taken year--</option>
                         <?php 
                                    $this_year = date("Y");
									$i=0;
                  ...
                  <div class="form-group">
                    ...
                    <textarea class="form-control" rows="5" maxlength="250" id="decriptilon" placeholder="Course's short decriptilon"  name="description"><?php if (isset($course) && is_numeric($course)){echo $row['description'];}elseif(isset($_POST['description'])){echo $_POST['description'];}?></textarea>
                  </div>

                  <div class="text-center">
                    <button  type="submit" name="save_course" class="btn mt-10 w-50p p-10 btn-primary fs-15 text-center  pull-center">Save</button>
                  </div>
                    </form>
                </div>
            </div> 
