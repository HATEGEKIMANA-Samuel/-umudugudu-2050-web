...
$documentTypeFH = mysqli_real_escape_string($connect, $_POST['documentTypeFH']);
...
$key_words = $givenName . '' . $familyName . $familyName . '' . $givenName . '' . $otherName . '' . $dob . '' . $birthplace;
$key_words = strtolower(preg_replace('/\s+/', '', $key_words));
$sql2 = "SELECT citizenId from citizen where documentNumber='$documentNumber' LIMIT 1";
if (isset($documentTypeFH) && !empty($documentTypeFH)) {
  $sql2 = "SELECT citizenId from citizen where documentNumber='$documentNumber' AND issuedCountry='$issuedCountry' LIMIT 1";
}
// passport info
$issuedCountry = mysqli_real_escape_string($connect, $_POST['issuedCountry']);
$checking = mysqli_fetch_all(mysqli_query($connect, $sql2), MYSQLI_ASSOC);
mysqli_begin_transaction($connect);
try {
  if (!isset($checking[0]["citizenId"])) {
    $query = "INSERT INTO citizen (documentNumber,is_family_heading,familyCategory,documentType,created_at,updated_at,status,key_words,issuedCountry) 
VALUES ('$documentNumber','1','head','$documentTypeFH','$date','$date','1','$documentNumber','$issuedCountry')";
    $results = mysqli_query($connect, $query);
    $lastid = mysqli_insert_id($connect);
  } else {
    $lastid = $checking[0]["citizenId"];
  }
  $query = "INSERT INTO citizen (givenName,familyName,otherName,gender,dob,martialstatus,ubudehe,birthplace, birthNationality,
 otherNationality, email, mobile, created_at, updated_at, documentType, status, 
 occupation, level_of_education, familyId,is_family_heading,
 familyCategory,key_words) VALUES ('$givenName','$familyName','$otherName','$gender','$dob','$martialstatus','$ubudehe','$birthplace','$birthNationality', '$otherNationality','$email','$mobile','$date','$date','$documentType','$status','$occupation','$level_of_education','$lastid','$is_family_heading','$familyCategory','$key_words')";
  $results = mysqli_query($connect, $query);

  $lastidcv = mysqli_insert_id($connect);

  $query = "INSERT INTO history (citizenId,movementType,isibo,upi,previousLocation,created_by,created_at,location) 
      VALUES('$lastidcv','$movementType','$isibo','$upi','$currentLocation','$created_by','$date', '$location')";

  $results = mysqli_query($connect, $query);

  $query = "INSERT INTO visitors (visited_by,resident_id,arrival_date,departure_date,created_at,updated_at,user_id,location,village,status) VALUES('$lastidcv','$familyId','$arrival_date','$departure_date','$date','$date','$created_by', '$location', '$currentLocation', '$status')";
