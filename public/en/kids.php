...
if (!isset($sp) || !is_numeric($sp)) {
	...
	$query = $database->query("SELECT * FROM members WHERE id ='$sp' AND status='1' LIMIT 1 ");
		...
		<div class="content mt-3">
			...
			<div class="col-lg-12">
				<div style="display: inherit; padding-bottom: 30px;" class="card">
					...
					echo "<li class='list-group-item info-list'><div class='labels'>Aho yavukiye</div><div class='datas'>{$spouse['birth_place']}</div></li>";
					...
					echo "<li class='list-group-item info-list'><div class='labels'>ID/Pasiporo</div><div class='datas'>{$spouse['document_id']}</div></li>";
					...
					echo "<li class='list-group-item info-list'><div class='labels'>Igihe yatangiwe</div><div class='datas'>{$spouse['issued_date']}</div></li>";
					echo "<li class='list-group-item info-list'><div class='labels'>Igihe izarangirira</div><div class='datas'>{$spouse['expiry_date']}</div></li>";
					echo "<li class='list-group-item info-list'><div class='labels'>Emeli</div><div class='datas'>{$spouse['email']}</div></li>";
					echo "<li class='list-group-item info-list'><div class='labels'>telefone</div><div class='datas'>{$spouse['phone']}</div></li>";
					$spouse['relationship'] = lang($spouse['relationship']);
					echo "<li class='list-group-item info-list'><div class='labels'>Isano </div><div class='datas'>{$spouse['relationship']}</div></li>";
					// new column
					echo "<li class='list-group-item info-list'><div class='labels'>Urwego ry'uburezi</div><div class='datas'>{$spouse['level_education']}</div></li>";

					echo "<li class='list-group-item info-list'><div class='labels'>Umwuga</div><div class='datas'>{$spouse['occupation']}</div></li>";
					// end of giba column
					echo "<li class='list-group-item info-list'><div class='labels'>Igihe yanditswe</div><div class='datas'>{$spouse['time']}</div></li>";
					$user  = $database->fetch_array($database->query("SELECT fname,lname FROM user WHERE id ='{$spouse['user']}' LIMIT 1 "));
					echo "<li class='list-group-item info-list last-child'><div class='labels'>Uwamwanditse</div><div class='datas'>{$user['fname']} {$user['lname']}</div></li>";


					echo '</ul>';

					$href = "add-member?kd=" . rawurlencode($kd_data[0]) . "&dpl=" . rawurlencode(encrypt_decrypt('encrypt', $spouse['head']));
					$back_href = "family?dpl=" . rawurlencode(encrypt_decrypt('encrypt', $spouse['head'])) . rawurlencode("%members");


					?>
					<a href="<?= $back_href ?>" class='edit-link'><button class='btn w-150 mb-10 border-radius-0  btn-primary btn-lg back_btn'>Garuka</button></a>
					...
					<?php
					$comment_query = $database->query("SELECT * FROM comments WHERE owner ='$sp' AND status='1' AND owner_type='kd' ORDER BY time ASC ");
					...
					<center>
						...
						<form id="comments" style="display: none" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
							...
							<input type="hidden" name="kids" value="<?php echo $sp; ?>" />
