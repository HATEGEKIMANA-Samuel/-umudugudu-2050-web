        ...
        switch (userLevel) {
          case 6:
            ...
            if (textLocation == userLevelText) {
              ...
              var sectorCode = locationArray[0] + '#' + locationArray[1] + '#' + locationArray[2]
              ...
              if (locationArray.length == 2) {
                ...
                setTimeout(() => { getCells(CLnumeric, textLocation) }, 100)
              } else if (locationArray.length == 4) {
                getSectors(districtCode, CLtext.split(' / ')[2])
                setTimeout(() => { getCells(sectorCode, CLtext.split(' / ')[3]) }, 100)
                setTimeout(() => { getVillages(CLnumeric, textLocation) }, 200)
              }
            }
            break
          case 5:
            ...
            if (textLocation == userLevelText) {
              ...
              if (locationArray.length == 3) {
                ...
                setTimeout(() => { getVillages(CLnumeric, textLocation) }, 100)
              }
            }
            break
          case 4:
          ...
          default:
            ...
            if (textLocation == userLevelText) {
              ...
              if (locationArray.length == 1) {
                ...
                getDistricts(locationArray[0], CLtext.split(' / ')[1])
                setTimeout(() => { getSectors(CLnumeric, textLocation) }, 100)
              } else if (locationArray.length == 3) {
                getDistricts(locationArray[0], CLtext.split(' / ')[1])
                setTimeout(() => { getSectors(districtCode, CLtext.split(' / ')[2]) }, 100)
                setTimeout(() => { getCells(CLnumeric, textLocation) }, 200)
              } else if (locationArray.length == 4) {
                getDistricts(locationArray[0], CLtext.split(' / ')[1])
                setTimeout(() => { getSectors(districtCode, CLtext.split(' / ')[2]) }, 100)
                setTimeout(() => { getCells(sectorCode, CLtext.split(' / ')[3]) }, 200)
                setTimeout(() => { getVillages(CLnumeric, textLocation) }, 300)
              }
            }
            break
        }
      } else { // FIXME: delete section
        var response = ajax.responseText.split('.')
...
function displayCLsession () { // FIXME: delete this section
  var ajax = ajaxObj('POST', 'includes/ajax_calls.php')
  ajax.onreadystatechange = () => {
    if (ajaxReturn(ajax) == true) {
      console.log(ajax.responseText)
