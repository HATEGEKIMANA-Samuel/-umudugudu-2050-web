...
$app_date = htmlentities($database->escape_value($_POST['start_date']));
$ret_date = htmlentities($database->escape_value($_POST['end_date']));
...
if (!isset($_POST['id_to_edit'])){
	...
	$query =$database->query("SELECT id FROM diplomats WHERE LOWER(passport)='$passporttocheck' AND status='1' AND passport !='' LIMIT 2 ");
	$found_diplomat  = $database->num_rows($query);
	if ($institution =='' || $title=='' || ($title_desc =='' && $title=='Other') || $given_name =='' || $family_name =='' || $gender =='' || $dob =='' || $marital_status =='' || $birth_place =='' || $birth_nationality =='' || $email =='') {
		$messages = "<span style='color:red;'>Please fill out all required fields</span>";
	}else if ($found_diplomat >0) {
		$messages = "<span style='color:red;'>Diplomat with $passport as passport is already registered</span>";
	} else {
		
	    $sql= "INSERT INTO diplomats(institution,title,other_title,given_name,family_name,other_name,gender,dob,marital_status,birth_place,birth_nationality,email,phone,rwandan_id,passport,issued_country,issued_date,expiry_date,father_name,father_country,mother_name,mother_country,time,user,type,app_date,ret_date) 
	                       VALUES($institution,'$title','$title_desc','$given_name','$family_name','$other_name','$gender','$dob','$marital_status','$birth_place',$birth_nationality,'$email','$phone','$rwandan_id','$passport','{$issued_country}','$issued_date','$expiry_date','$father_name',$father_country,'$mother_name',$mother_country,NOW(),{$_SESSION["id"]},'1','{$app_date}','{$ret_date}')";
	...
	$found_diplomat  = $database->num_rows($query);
	if ($institution =='' || $title=='' || ($title_desc =='' && $title=='Other') || $given_name =='' || $family_name =='' || $gender =='' || $dob =='' || $marital_status =='' || $birth_place =='' || $birth_nationality =='' || $email ==''  || $father_country ==''  || $mother_country =='') {
    ...
    $sql = "UPDATE diplomats SET institution=$institution,title='$title',other_title='$title_desc',given_name='$given_name',family_name='$family_name',other_name='$other_name',gender='$gender',dob='$dob',marital_status='$marital_status',birth_place='$birth_place',birth_nationality=$birth_nationality,email='$email',phone='$phone',rwandan_id='$rwandan_id',passport='$passport',issued_country='{$issued_country}',issued_date='$issued_date',expiry_date='$expiry_date',father_name='$father_name',father_country=$father_country,mother_name='$mother_name',mother_country=$mother_country, app_date='{$app_date}', ret_date='{$ret_date}' WHERE id=$id";
    if ($database->query($sql)) {
        $id=encrypt_decrypt('encrypt', $id);
        header("location:diplomats?dpl=$id");
    }
        ...
        <div class="tab-container">
            <div class="tab-content mt-10" id="nav-tabContent">
                ...
                <div class="tab-content-body">
                   <h4 class="text-center mt-20 fs-18">Local Staff Registration Form <hr class="mt-20 mb-30"></h4>
...
<!--                     <div class="col-lg-4"> -->
                   ...
                   <?php 
                    ...
                    <div class="col-lg-4 <?php echo $style ?>">
                      <div id="title_desc" class="form-group">
                        <label for="name">Other Title<span class="required-mark">*</span></label>
                    ...
                    <div class="col-lg-12">
                      <fieldset class="fiedset-type pb-20">
                        <legend class="fieldset-legend">Contract Period</legend>
                        <div class="col-lg-6">
                          <div class="form-group">
                            <label for="name">Starting Date</label>
                          <input type="text"  maxlength="20" autocomplete="off" placeholder="Starting Date" class="form-control datepicker"  name="start_date" value="<?php if (isset($diplomat) && is_numeric($diplomat)){echo $row['app_date'];}elseif(isset($_POST['start_date'])){echo $_POST['start_date'];}?>">
                          </div>
                        </div>
                          ...
                          <div class="col-lg-6">
                              ...
                              <input type="text"  maxlength="20" autocomplete="off" placeholder="Ending Date" class="form-control datepicker1"  name="end_date" value="<?php if (isset($diplomat) && is_numeric($diplomat)){echo $row['ret_date'];}elseif(isset($_POST['end_date'])){echo $_POST['end_date'];}?>">
                            </div>
                          </div>
                      </fieldset>
                    </div>
                    ...
                    <div class="col-lg-12">
                        ...
                        <fieldset class="fiedset-type pb-20">
                          <legend class="fieldset-legend">Father</legend> 
                        <div class="col-lg-6">
                          <div class="form-group">
                    ...
                    <div class="col-lg-12">
                      ...
                      <fieldset class="fiedset-type pb-20">
                          <legend class="fieldset-legend">Mother</legend> 
                    ...
                    <div class="col-lg-offset-4 col-lg-4">
                      <button  type="submit" name="save_diplomat" class="btn btn-primary pt-10 pb-10 fs-15 w-100p pull-right">Save</button>
                    </div>
                  	...
                  	<!-- <div class="form-group"> -->
                            ...
                            <!-- <select  class="form-control" onchange="toggleTitle('title')" id="title" name="title"> -->
                                <!-- <option value="">--SELECT--</option> -->
	                              <?php 
           //                      $query1 = $database->query("SELECT id,name FROM title ORDER BY id ASC");
								   //       $i=0;
					      //                while($row1  = $database->fetch_array($query1)) {
