...
//   var ajax = ajaxObj_("POST", "includes/ajax_calls.php");
...
//         if (item.split(",")[1]) {
//           html += `<tr><td class="text-left fs-14" onclick="getDistricts(${
//             item.split(",")[0]
//           }, \'${
//             item.split(",")[1]
...
//         if (item.split(",")[1]) {
//           html += `<tr><td class="text-left fs-14" onclick="getSectors('${province}#${
//             item.split(",")[0]
//           }', \'${
//             item.split(",")[1]
//           }\')"><span class="legend-label ball-2"></span>${
...
//         if (item.split(",")[1]) {
//           html += `<tr><td class="text-left fs-14" onclick="getCells('${district}#${
//             item.split(",")[0]
//           }', \'${
//             item.split(",")[1]
//           }\')"><span class="legend-label ball-2"></span>${
...
//       $("#district-back").attr(
//         "onclick",
//         "getDistricts('" + provinceId + "', '" + provinceName + "')"
//       );
...
//       cells.forEach((item) => {
//         if (item.split(",")[1]) {
//           html += `<tr><td class="text-left fs-14" onclick="getVillages('${sector}#${
//             item.split(",")[0]
//           }', \'${
//             item.split(",")[1]
//           }\')"><span class="legend-label ball-2"></span>${
...
//       $("#sector-back").attr(
//         "onclick",
//         "getSectors('" + districtId + "', '" + districtName + "')"
//       );
...
function changeTotolNumber(office_code) {
  $(".updatec").html(
    '<i class="fa fa-spinner fa-spin " aria-hidden="true"></i><span class="sr-only">Loading...</span>'
  );
  $.post("controller/familyController.php", {
    office: office_code,
    action: "updatetotal",
  })
    .done(function (data) {
      //console.log(data);
      fillNumbers(JSON.parse(data));
    })
    .fail(function (err) {
      console.log(err);
    });
}
function fillNumbers(obj) {
  $(".ubwicanyic").html(obj.ubwicanyi);
  $(".fraudc").html(obj.fraud);
  $(".malec").html(obj.male);
  $(".violancec").html(obj.violance);
  $(".visitorc").html(obj.visitors);
  $(".urugomoc").html(obj.urugomo);
  $(".umupakac").html(obj.umupaka);
  $(".ubujurac").html(obj.ubujura);
  $(".rentc").html(obj.rent);
  $(".notrentc").html(obj.notrent);
  $(".kwiyahurac").html(obj.kwiyahura);
  $(".intwaroc").html(obj.intwaro);
  $(".ibizac").html(obj.ibiza);
  $(".femalec").html(obj.female);
  $(".drugc").html(obj.drug);
  $(".covidc").html(obj.covid);
  $(".conflictc").html(obj.conflict);
  $(".peoplec").html(obj.people);
}
